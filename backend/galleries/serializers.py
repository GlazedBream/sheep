...
class GetPresignedUrlRequestSerializer(serializers.Serializer):
    file_name = serializers.CharField(
        max_length=255,
        required=True,
        help_text="업로드할 파일의 이름"
    )
    file_type = serializers.CharField(
        max_length=100,
        required=True,
        help_text="파일의 MIME 타입"
    )

    class Meta:
        extra_kwargs = {
            'file_name': {'example': 'event_photo1.jpg'},
...
class GetPresignedUrlResponseSerializer(serializers.Serializer):
    ...
    class Meta:
        extra_kwargs = {
            'upload_url': {
                'example': 'https://sheep-diary-photos.s3.ap-northeast-2.amazonaws.com/temp_uploads/uuid/event_photo1.jpg?AWSAccessKeyId=...'
            },
            's3_key': {'example': 'temp_uploads/uuid/event_photo1.jpg'}
        }
...
class ConfirmImageResponseSerializer(serializers.Serializer):
    ...
    class Meta:
        extra_kwargs = {
            'final_url': {
                'example': 'https://sheep-diary-photos.s3.ap-northeast-2.amazonaws.com/uploads/2025/05/02/event_photo1.jpg'
            },
